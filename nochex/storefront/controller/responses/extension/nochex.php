...
class ControllerResponsesExtensionNochex extends AController {
	public function main() {
		...
		$template_data['api_key'] = $this->config->get('nochex_api');
		...
		if ($this->config->get('nochex_test') == 1){
			$template_data['test'] = 1;
		}else {
			$template_data['test'] = 0;
		}
		...
		/*if ($this->config->get('nochex_api') <> "") {*/
			$this->processTemplate('responses/api.tpl');			
		/*} else {
